...
function normalizeStoredPayoutStatus(raw) {
    const status = String(raw || '').trim();
    if (['Approved', 'APPROVED'].includes(status)) return 'Pending';
    if (['Paid', 'PAID'].includes(status)) return 'Released';
...
function resolvePayoutStatus(payout, ctx) {
    ...
    const storedRaw = String(payout.status || '');
    const stored = normalizeStoredPayoutStatus(storedRaw);
    ...
    if (['Approved'].includes(storedRaw)) return PAYOUT_STATUS.APPROVED;
...
async function reconcilePayoutRecord(payout, ctx) {
    const Payout = require('../models/Payout');
    const storedRaw = String(payout.status || '');
    const normalizedStored = normalizeStoredPayoutStatus(storedRaw);
    ...
    if (normalizedStored !== storedRaw) {
        updates.status = normalizedStored;
    }
