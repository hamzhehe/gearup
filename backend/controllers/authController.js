...
const client = new OAuth2Client(process.env.GOOGLE_CLIENT_ID || '977790937405-ks3rj67mmet2pgmmi0v1occf7b9vpdsa.apps.googleusercontent.com');
const sendEmail = require('../utils/sendEmail');
const { getRegistrationOTPTemplate, getResendOTPTemplate, getPasswordResetTemplate } = require('../utils/emailTemplates');
...
exports.register = async (req, res, next) => {
    try {
        ...
        try {
            await sendEmail({
                email: user.email,
                subject: '🎉 Verify Your GearUp Account',
                html: getRegistrationOTPTemplate(otp, name)
            });
        } catch (err) {
...
exports.resendOTP = async (req, res, next) => {
    try {
        ...
        await sendEmail({
            email: user.email,
            subject: '🔄 Your New GearUp Verification Code',
            html: getResendOTPTemplate(otp)
        });
...
exports.submitVerification = async (req, res, next) => {
    try {
        ...
        console.log('[AUTH] Verification document uploaded to Cloudinary', {
...
exports.forgotPassword = async (req, res, next) => {
    try {
        ...
        try {
            await sendEmail({
                email: user.email,
                subject: '🔐 Reset Your GearUp Password',
...
exports.googleAuth = async (req, res, next) => {
    try {
        ...
        const ticket = await client.verifyIdToken({
            idToken: token,
            audience: process.env.GOOGLE_CLIENT_ID || '977790937405-ks3rj67mmet2pgmmi0v1occf7b9vpdsa.apps.googleusercontent.com',
        });
