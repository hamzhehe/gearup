            ...
            data: {
                ...
                payouts: {
                    ...
                    holding: payouts.filter((p) => {
                        ...
                        const normalized = normalizeStoredPayoutStatus(status);
                        return !operationsSummary.refundedOrderIds.includes(oid)
                            && !['Released', 'Failed', 'Cancelled'].includes(normalized);
                    }).length,
                    approved: payouts.filter((p) => {
                        const status = String(p.status || '').trim();
                        const normalized = normalizeStoredPayoutStatus(status);
                        return ['Released'].includes(normalized);
                    }).length,
                    refunded: operationsSummary.refundedOrders,
                },
                escrow: {
...
exports.markPayoutAsPaid = async (req, res, next) => {
    try {
        ...
        if (payout.status === 'Released' || payout.releasedAt) {
            return res.status(400).json({ success: false, error: 'Payout is already marked as released' });
        }
