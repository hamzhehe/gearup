...
exports.getAdminPayouts = async (req, res) => {
    try {
        ...
        function buildStatusFilter(statusValue) {
            if (!statusValue || statusValue === 'all') return null;
            const normalized = String(statusValue || '').trim();
            switch (normalized) {
                case 'Pending':
                    return { $in: ['Pending', 'Approved'] };
                case 'Held':
                    return { $in: ['Held', 'Holding'] };
                case 'Released':
                    return { $in: ['Released', 'Paid'] };
                case 'Failed':
                    return 'Failed';
                case 'Cancelled':
        ...
        const normalizedPayouts = filteredPayouts.map(p => {
            ...
            if (['Approved', 'APPROVED'].includes(status)) {
                obj.status = 'Pending';
            } else if (['Paid', 'PAID'].includes(status)) {
                obj.status = 'Released';
        ...
        allPayouts.forEach(p => {
            let st = String(p.status || '').trim();
            if (['Approved', 'APPROVED'].includes(st)) st = 'Pending';
            if (['Paid', 'PAID'].includes(st)) st = 'Released';
...
exports.releasePayout = async (req, res) => {
    try {
        console.log('[PAYOUT] Release request:', {
        ...
        if (payout.status === 'Released' || payout.releasedAt) {
            return res.status(409).json({ success: false, error: 'Payout has already been released' });
        }

        if (payout.status === 'Cancelled') {
            return res.status(400).json({ success: false, error: 'Cannot release a payout that has been cancelled' });
        }
        ...
        // Validate seller-specific delivery/completion status
        const orderStatus = (order.status || '').toLowerCase();
        const sellerStat = getSellerStat(order, payout.seller);
        const sellerStatus = String(sellerStat?.status || '').toLowerCase();
        const eligibleSellerStatus = ['completed', 'delivered'];
        if (!eligibleSellerStatus.includes(sellerStatus) && !['completed', 'delivered'].includes(orderStatus)) {
            return res.status(400).json({ success: false, error: `Seller payout can only be released after the seller portion of the order is Delivered or Completed (Current: ${sellerStat?.status || order.status})` });
        }
        ...
        // Atomically persist the release so a concurrent request cannot flip the
        // same payout to Released twice (exactly-once semantics).
        const releaseResult = await Payout.updateOne(
            { _id: payout._id, status: { $ne: 'Released' }, releasedAt: null },
            { $set: payout.toObject({ versionKey: false }) }
        );
        ...
        console.log('[PAYOUT] Released:', {
            ...
            amountEur: conversionData.amountEur
        });
