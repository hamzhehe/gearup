...
exports.createProduct = async (req, res, next) => {
    try {
        ...
        if (!req.body.subcategory || !String(req.body.subcategory).trim()) {
            return res.status(400).json({ success: false, error: 'Please select a subcategory.' });
        }
        if (!isValidCategorySubcategory(req.body.category, req.body.subcategory)) {
            return res.status(400).json({ success: false, error: `The selected subcategory "${req.body.subcategory}" does not belong to the selected category "${req.body.category}".` });
        }
    ...
    try {
        ...
        if (updatePayload.category !== undefined || updatePayload.subcategory !== undefined) {
            ...
            if (!isValidCategorySubcategory(targetCategory, targetSubcategory)) {
                return res.status(400).json({ success: false, error: `The selected subcategory "${targetSubcategory}" does not belong to the selected category "${targetCategory}".` });
            }
        }
