...
const OrdersTable = ({
  ...
  // Close the dropdown when the page scrolls or resizes so its fixed
  // position never points at stale coordinates.
  useEffect(() => {
                    ...
                    <td className="px-6 py-4 text-right relative">
                      <button 
                        onClick={(e) => {
                          ...
                          // The dropdown is rendered in a portal (document.body) so it is
                          // never clipped by the table's overflow-x-auto scroll container.
                          // Measure the button so we can pin the fixed dropdown to it.
                          const rect = e.currentTarget.getBoundingClientRect();
                          const dropdownWidth = 208; // w-52
                          let left = rect.right - dropdownWidth + 24; // matches the old right-6 alignment
                      ...
                      {openDropdownId === order.id && dropdownPos && typeof document !== 'undefined' && createPortal(
                        <div 
                          ref={dropdownRef}
                          className="fixed w-52 bg-white border border-slate-200/80 shadow-2xl rounded-2xl z-[9999] py-2 animate-in fade-in zoom-in-95 duration-200"
                          style={{ top: dropdownPos.top, left: dropdownPos.left, boxShadow: '0 20px 60px rgba(0,0,0,0.12), 0 4px 12px rgba(0,0,0,0.06)' }}
                        >
                          <button 
