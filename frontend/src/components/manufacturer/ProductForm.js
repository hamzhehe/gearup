...
import { CATEGORIES, getSubcategoriesForCategory, isValidCategorySubcategory } from '@/lib/categories';
...
const ProductForm = ({ id }) => {
    ...
    const computeBulkPriceFromSingle = (singlePrice, packSize) => {
        const parsedSingle = Number(singlePrice);
        const parsedPack = Number(packSize);
        if (!Number.isFinite(parsedSingle) || parsedSingle <= 0) return '';
        if (!Number.isFinite(parsedPack) || parsedPack <= 0) return '';
        return parsedSingle * parsedPack;
    };

    const deriveSinglePriceFromBulk = (bulkPrice, packSize) => {
        const parsedBulk = Number(bulkPrice);
        const parsedPack = Number(packSize);
        if (!Number.isFinite(parsedBulk) || parsedBulk <= 0) return '';
        if (!Number.isFinite(parsedPack) || parsedPack <= 0) return '';
        return String(Math.round(parsedBulk / parsedPack));
    };
    ...
    const getValidationErrors = () => {
        ...
        } else if (formData.category && !isValidCategorySubcategory(formData.category, formData.subcategory)) {
            errors.subcategory = 'The selected subcategory does not belong to the selected category.';
        }
    ...
    const bulkUnitSinglePricePreview = !dozenPricingMode && derivedBulkPrice !== ''
        ? Number(singleUnitPrice)
        : null;
    const bulkUnitPrice = !dozenPricingMode && derivedBulkPrice !== ''
        ? derivedBulkPrice
        : formData.pricePerBulkUnit;
    ...
    const fetchProduct = useCallback(async () => {
        try {
            ...
            if (data.success) {
                ...
                // Load the existing subcategory only when it is valid for the loaded category.
                // Legacy values (e.g. the old generic "Cricket Bat") fall back to the
                // "Select Subcategory" placeholder so the seller can re-classify manually.
    ...
    const handleChange = (e) => {
        ...
        if (name === 'name') {
            ...
            // Changing the main category always resets the subcategory so a
            // previous category's subcategory can never stay selected.
            setFormData(prev => ({
                ...prev,
                category: value,
                subcategory: ''
    ...
    const handleSingleUnitPriceChange = (e) => {
        ...
        setFormData(prev => ({
            ...prev,
            pricePerBulkUnit: computeBulkPriceFromSingle(value, prev.packSize),
        }));
    };
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={handleChange}
                                        className={`w-full h-[52px] px-4 bg-[#FFFFFF] border rounded-[14px] focus:ring-[4px] outline-none transition-all duration-200 font-sans text-[#0F172A] text-[15px] cursor-pointer appearance-none ${validationErrors.category
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <span className="text-[10px] bg-[#E8FFF5] text-[#00A878] px-2.5 py-1 rounded-full font-[700] uppercase tracking-widest border border-[#00A878]/20">
                                    {formData.category || 'Select a category'}{formData.subcategory ? ` • ${formData.subcategory}` : ''}
                                </span>
                                {formData.sku && (
                                ...
                                {(dozenPricingMode && dozenPreviewTotal > 0) ? (
                                    ...
                                    <div className="rounded-[16px] border border-[#E2E8F0] bg-[#F8FAFC] p-4 space-y-2">
                                        ...
                                        <div className="flex items-center justify-between text-[13px] font-[700] text-[#0F172A]">
                                            ...
                                            <span className="text-[#00A878]">{formatPKR(bulkUnitPrice)}</span>
                                        </div>
                                    </div>
                                ) : null}
                            </>
