...
export async function fetchSecureVerificationDocumentBlobUrl(userId) {
  const token = getAuthToken();
  if (!token) {
    throw new Error('You must be signed in to view this document.');
  }

  const url = new URL(`${getApiBaseUrl()}/api/auth/verification-document`);
  if (userId) {
...
export function getSecureVerificationDocumentUrl(userId) {
  const token = getAuthToken();
  if (!token) {
    throw new Error('You must be signed in to view this document.');
  }
...
export async function openSecureVerificationDocument(userId) {
  const token = getAuthToken();
  if (!token) {
    throw new Error('You must be signed in to view this document.');
  }
